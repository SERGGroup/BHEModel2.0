...
def label_line(line, x, label=None, align=True, **kwargs):
    ...
    if align:
        ...
        ang = abs(degrees(atan2(dy, dx)))
        sign = np.sign(dx*dy)

        # Transform to screen co-ordinates
        pt = np.array([x, y]).reshape((1, 2))
        trans_angle = sign * ax.transData.transform_angles(np.array((ang,)), pt)[0]
...
def label_lines(lines, align=True, xvals=None, **kwargs):
    ...
    # Take only the lines which have labels support than the default ones
    for line in lines:
        label = line.get_label()
