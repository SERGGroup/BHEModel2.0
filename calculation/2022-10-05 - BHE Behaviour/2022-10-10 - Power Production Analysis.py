...
CO2_input.set_variable("rho", 900)
...
for i in range(len(t_in_points)):
    ...
    for j in range(len(depth_points)):
        ...
        if 0 <= bhe_CO2.eta_II <= 1 and P_out < bhe_CO2.points[-1].get_variable("P"):
...
for i in range(len(t_in_points)):
    ...
    for j in range(len(depth_points)):
        ...
        if 0 <= bhe_water.eta_II <= 1:
            ...
            y_water.append(bhe_water.eta_II * 0.2)
            pbar.update(1)
...
ax.set_ylim(bottom=0.01, top=0.06)
