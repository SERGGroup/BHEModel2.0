...
# %%-------------------------------------   CALCULATE                           -------------------------------------> #
p_rel_curr = 1 / 200
in_state = PlantThermoPoint(["Water"], [1], unit_system="MASS BASE SI")
...
for i in range(len(t_rel_list)):
    ...
    for k in range(len(dz_nd_list)):
        ...
        axs[0].plot(x_values[:, k], spc_work_liq[:, k], "-", label=label_curr, color=color)
        axs[1].plot(x_values[:, k], spc_ex_liq[:, k], "-", label=label_curr, color=color)
        axs[2].plot(x_values[:, k], ex_eta_liq[:, k], "-", label=label_curr, color=color)
