...
def update_values(in_state, in_base_state, t_rel, p_in, grad_nd_nd, dz_nd):

    t_amb = t_rel * in_state.RPHandler.TC
    ...
    grad_nd = grad_nd_nd + grad_nd_lim

    grad = grad_nd * scipy.constants.g / cp_in
    depth = dz_nd * (cp_in * in_state.get_variable("T")) / scipy.constants.g
    t_rocks = t_amb + grad * depth
...
n_temp = 5
p_log10_max = 1
t_rels = np.linspace(np.log10(0.5), np.log10(2), n_temp)
...
for i in range(n_temp):
    ...
    bhe_in.set_variable("T", t_rels[i] * bhe_in.RPHandler.TC)
    ...
    for j in range(n_points):
        ...
        base_surface = update_values(
            ...
            bhe_in, bhe_in_base_si,
            t_rel=t_rels[i], p_in=p_curr,
            grad_nd_nd=grad_nd_nd, dz_nd=dz_nd
