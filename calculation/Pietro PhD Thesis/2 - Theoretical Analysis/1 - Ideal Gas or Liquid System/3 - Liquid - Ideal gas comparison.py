...
dir_exp_perc_abs_min = dz_nd / (1 + dz_nd*grad_nd)
dir_exp_perc_abs_max = dz_nd/((1 + dz_nd)*(1 + dz_nd*grad_nd - dz_nd))
...
for i in range(len(dz_nd_list)):
    ...
    axs[1, 1].plot((grad_nd[i, :] - 1), dir_exp_perc_rel_min[i, :], "-", color=color)
    lines.append(
    ...
    axbig.plot((grad_nd[i, :] - 1), dir_exp_perc_abs_min[i, :], "-", color=color)
    big_lines.append(
...
x_values = [grad_nd.ravel() - 1, dz_nd_ext, grad_nd.ravel() - 1]
