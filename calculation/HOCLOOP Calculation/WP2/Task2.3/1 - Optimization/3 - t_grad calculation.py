...
# %%------------   CALCULATION                            -----------------------------------------------------------> #
for i in range(len(t_grad_list)):

    t_grad = t_grad_list[i]
    ...
    pbar = tqdm(desc="Calculate t_grad={}".format(t_grad), total=len(time_points))
    ...
    file_path = os.path.join(RES_FOLDER, "optimization t_grad={:.0f}.xlsx".format(t_grad*1000))
